...
class AdminController {
  ...
  static async getWakafById(req, response) {
    try {
      ...
      response.status(200).json("success get all wakaf data");
    } catch (error) {
      response.status(500).json(error);
    }
  }
  ...
  static async setDistribution(req, response) {
    try {
      ...
      if (isReceive && !getData.status) {
        ...
        const receive = await Wakaf.update(
          { status: "true" },
          {
            where: {
              id: getData.id,
      ...
      response.status(200).json("succes change status distribution");
    } catch (error) {
      response.status(500).json(error);
    }
  }
  ...
  static async updateFund(req, response) {
    try {
      ...
      const updatedCapital = await fund.update(
        { capital: calculation },
        {
          where: {
            obtained_at: dateInYear,
      ...
      response.status(200).json("succes update fund");
    } catch (error) {
      response.status(500).json(error);
    }
  }
